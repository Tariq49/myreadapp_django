...
CREATE TABLE book (
    ...
    category book_category_enum NOT NULL,  -- Assuming book_category_enum is defined elsewhere
    published_date DATE NOT NULL,
    publisher VARCHAR(50) NOT NULL,
    authors VARCHAR(50) ARRAY NOT NULL,  -- Stores author names as an array
    lang VARCHAR(50) NOT NULL,
    edition INT,
    format book_format_enum NOT NULL   -- Assuming book_format_enum is defined elsewhere
);
...
CREATE TABLE reader(
   ...
   CHECK (title IN ('Mrs', 'Mr', 'Dr', 'Ms'))
);

CREATE TABLE my_read (
  id SERIAL PRIMARY KEY,
  book_isbn VARCHAR(13) NOT NULL REFERENCES book(isbn), 
  reader_username VARCHAR(50) NOT NULL REFERENCES reader(username), 
  percentage_read INT , 
  start_read_date DATE,
  end_read_date DATE,
  
  CHECK (percentage_read BETWEEN 0 AND 100),
  CHECK (start_read_date < end_read_date),
...
VALUES ('[0,0]','pending'),('[1,99]','reading'),('[100,100]','done');
